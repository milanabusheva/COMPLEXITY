...
// берем шаг HillClimber из статьи, делая в качестве шага 2-opt с каким-то ребром
void hcStep(const std::vector<Point>& points, std::vector<int>& path, double& length,
    ...
    if (j - i < 2 || (i == 0 && j == static_cast<int>(n) - 1)) return; // не получится поменять ребра
...
int main(int argc, char* argv[]) {
    ...
    const int POP_SIZE = 10; // уменьшила по сравнению со значением из статьи т.к. дает лучше результат
    const double T_START = 10.0;
    const double T_END = 0.01;
    const double TIME_LIMIT_SECONDS = 10.0 * 60.0;
    std::mt19937 rng(42);
    ...
    for (auto curTime = std::chrono::duration<double>(std::chrono::steady_clock::now() - start).count();
          curTime <= TIME_LIMIT_SECONDS;) {

        // T постепенно снижаем линейно... по сути делаем отжиг
        double progress = curTime / TIME_LIMIT_SECONDS;
    ...
    for (size_t i = 0; i < population[bestIdx].size(); ++i) {
        std::cout << population[bestIdx][i] << ' ';
    }
    std::cout << '\n';
}